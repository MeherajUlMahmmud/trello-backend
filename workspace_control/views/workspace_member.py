from django_filters.rest_framework import DjangoFilterBackend
from rest_framework.filters import SearchFilter
from rest_framework.response import Response
from rest_framework.status import HTTP_200_OK, HTTP_201_CREATED, HTTP_403_FORBIDDEN

from common.custom_permissions import AdminOrStaffUserPermission
from common.custom_view import (
    CustomCreateAPIView, CustomUpdateAPIView, CustomRetrieveAPIView, CustomListAPIView,
)
from workspace_control.models import WorkspaceMemberModel
from workspace_control.serializers.workspace_member import WorkspaceMemberModelSerializer


class GetWorkspaceMemberListAPIView(CustomListAPIView):
    queryset = WorkspaceMemberModel.objects.filter(is_active=True, is_deleted=False)
    permission_classes = [AdminOrStaffUserPermission]
    serializer_class = WorkspaceMemberModelSerializer.List
    filter_backends = [SearchFilter, DjangoFilterBackend]
    search_fields = ['role', 'workspace__title',
                     'user__first_name', 'user__last_name']


class GetWorkspaceMemberDetailsAPIView(CustomRetrieveAPIView):
    queryset = WorkspaceMemberModel.objects.filter(is_active=True, is_deleted=False)
    serializer_class = WorkspaceMemberModelSerializer.List

    def retrieve(self, request, *args, **kwargs):
        instance = self.get_object()
        requested_user = request.user

        if not request.user.check_object_permissions(request, instance):
            return Response({
                'detail': 'You don\'t have permission to perform this action.'
            }, status=HTTP_403_FORBIDDEN)

        serializer = WorkspaceMemberModelSerializer.List(instance)
        return Response({
            'data': serializer.data,
        }, status=HTTP_200_OK)


class CreateWorkspaceMemberAPIView(CustomCreateAPIView):
    permission_classes = (AdminOrStaffUserPermission,)
    queryset = WorkspaceMemberModel.objects.filter(is_active=True, is_deleted=False)
    serializer_class = WorkspaceMemberModelSerializer.Write

    def post(self, request, *args, **kwargs):
        serializer = self.serializer_class(data=request.data)
        serializer.is_valid(raise_exception=True)
        validated_data = serializer.validated_data

        workspace_member = WorkspaceMemberModel.objects.create(
            created_by=request.user,
            **validated_data
        )
        workspace_member.save()

        return Response({
            'message': 'Workspace Member added successfully.',
        }, status=HTTP_201_CREATED)


class UpdateWorkspaceMemberDetailsAPIView(CustomUpdateAPIView):
    queryset = WorkspaceMemberModel.objects.filter(is_active=True, is_deleted=False)
    serializer_class = WorkspaceMemberModelSerializer.Write

    def patch(self, request, *args, **kwargs):
        instance = self.get_object()
        requested_user = request.user
        if not request.user.check_object_permissions(request, instance):
            return Response({
                'message': 'You don\'t have permission to perform this action.'
            }, status=HTTP_403_FORBIDDEN)

        serializer = self.serializer_class(
            instance, data=request.data, partial=True)
        serializer.is_valid(raise_exception=True)
        serializer.save(
            updated_by=request.user,
        )
        return Response(serializer.data, status=HTTP_200_OK)
